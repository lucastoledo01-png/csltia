...
export function NewsletterRenderer({
  title,
  subtitle,
  date = "QUINTA-FEIRA, 20 DE AGOSTO DE 2026",
  category = "Radar",
  readTime = "5 min",
  coverImage,
  contentHtml,
  ...
  const whatsappShareUrl = `https://api.whatsapp.com/send?text=${encodeURIComponent(
    `Confira esta edição do Casaloti IA: ${title}`
  )}`;
  ...
  return (
    <article className="mx-auto max-w-2xl bg-white px-4 py-8 text-black sm:px-6 md:py-12 shadow-[0_4px_30px_rgba(0,0,0,0.03)] rounded-[32px] border border-[#eaecf0]">
        ...
        <div className="mt-6 flex justify-center">
          <div className="rounded-2xl bg-[#ff4a1c] px-6 py-2 shadow-lg shadow-[#ff4a1c]/20">
            <span className="font-mono text-3xl font-black lowercase tracking-tighter text-white sm:text-4xl">
              the news <span className="text-[#fef08a]">/ casaloti</span>
            </span>
          </div>
        </div>
      ...
      {/* 2. Bloco EM 60 SEGUNDOS (se houver ou padrão) */}
      <section className="my-8 rounded-[24px] border border-[#fef08a] bg-[#fffde7] p-6 shadow-sm">
        ...
        <div className="mt-3 space-y-2 text-sm font-medium text-[#344054]">
          <p>
            <mark className="bg-[#fef08a] px-1 font-bold text-black">O radar diário de IA</mark>: As principais novidades de modelos, ferramentas e mercado filtradas para você testar no mesmo dia.
          </p>
        </div>
      </section>
      ...
      {/* Citação Destacada (Estilo The News) */}
      {quote ? (
      ...
      {/* 3. Conteúdo Principal da Edição (HTML Fluido ou Seções) */}
        ...
        <div className="my-8 space-y-8">
          {sections?.map((sec, idx) => (
            <section key={idx} className="space-y-4">
              ...
              {/* Botão de Compartilhar no WhatsApp para cada seção */}
              <div className="pt-2">
                <a
      ...
      {/* 4. Termômetro de Imparcialidade / Pesquisa da Edição */}
      <section className="my-10 rounded-[28px] border border-[#d0d5dd] bg-[#fafafa] p-6 text-center shadow-sm">
        ...
        <h3 className="mt-2 text-xl font-black text-black">
          Na sua opinião, a edição de hoje foi imparcial?
        </h3>
        ...
        {pollVoted ? (
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-3 text-xs font-bold">
            <button
              onClick={() => setPollVoted("Sim, foi imparcial")}
              className="rounded-xl border border-[#d0d5dd] bg-white p-3 hover:border-[#ff4a1c] hover:bg-[#fff5f2] transition-colors"
            >
              🟢 Sim, foi imparcial
            </button>
            <button
              onClick={() => setPollVoted("Parcial para a direita")}
              className="rounded-xl border border-[#d0d5dd] bg-white p-3 hover:border-[#ff4a1c] hover:bg-[#fff5f2] transition-colors"
            >
              🟡 Parcial para a direita
            </button>
            <button
              onClick={() => setPollVoted("Parcial para a esquerda")}
              className="rounded-xl border border-[#d0d5dd] bg-white p-3 hover:border-[#ff4a1c] hover:bg-[#fff5f2] transition-colors"
            >
              🔵 Parcial para a esquerda
            </button>
          </div>
        )}
      </section>
      ...
      <footer className="mt-12 border-t border-[#eaecf0] pt-8 text-center text-xs text-[#667085]">
        <p className="font-bold text-black">Casaloti IA — Jornal de IA sem fumaça e sem palestra de LinkedIn.</p>
        <p className="mt-2">Direito sua caixa de entrada e no site todos os dias.</p>
      </footer>
    </article>
  );
}
